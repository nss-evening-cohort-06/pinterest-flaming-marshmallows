...
app.service("PinterestService", function ($http, $q, $rootScope, FIREBASE_CONFIG) {
  const getBoards = (userUid) => {
    ...
    return $q((resolve, reject) => {
      $http.get(`${FIREBASE_CONFIG.databaseURL}/pinterest/boards.json?orderBy="uid"&equalTo="${userUid}"`).then((results) => {
  ...
  const getPins = (userUid) => {
    let pins = [];
    return $q((resolve, reject) => {
      $http.get(`${FIREBASE_CONFIG.databaseURL}/pinterest/pins.json?orderBy="uid"&equalTo="${userUid}"`).then((results) => {
